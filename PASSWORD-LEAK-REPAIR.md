...
| **Residual — public legacy repo** | 🟡 Hygiene only. `albapepper/Scoracle` (PUBLIC, `API_SPORTS_KEY` ×7 commits) is superseded by `scoracle/scoracle-backend`; Scott chose to delete it. **Its one secret is revoked, so nothing live is exposed.** Blocked on `gh auth refresh -h github.com -s delete_repo` (interactive, must run on the Mac — the token lives in that machine's keyring; any directory). |
