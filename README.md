...
- **Application adapters:** `rust/src/application/analyst.rs` and `influencer.rs` own their characters' retrieval, memory preparation, work policy, and claim-aware publication. `rust/src/application/scout.rs` now owns Scout queue versions, rerun triggers, persistence, exact-claim completion, and follow-up coordination while its larger evidence/creation core remains in `junctions/scout`. `rust/src/application/outbox.rs` durably reconciles their concrete Momentum/Oracle obligations. `runtime/route.rs` selects/governs model transports. `runtime/worker.rs` and `runtime/work.rs` retain the current queue lifecycle.
- **Other character work:** still uses `junctions/` and the database-bearing `runtime/harness.rs` compatibility context. Shared extraction delegates to Studio. This context is migration scaffolding; it is not a second permanent harness.
...
- **Durability:** migration 256 and `runtime/work.rs` give each running lease a UUID claim token plus its captured input revision. Migrations 257–259 make Influencer, Analyst, and Scout claim-aware publishers: after inference, each short transaction locks the exact lease, writes any product with required provenance, records only its durable follow-up obligation, and deletes the claim. A stale or superseded execution publishes nothing. Product-bearing Vibe and Rating completions offer Momentum before checking Oracle; Momentum completion and a debounced Rating need only the Oracle barrier. Analyst `NoMaterial` and Scout debounce write no product while still committing their respective barrier obligation and completion; Scout's no-stats marker remains a real product. Diagnostic ledger writes remain explicitly best-effort after commit. Other handlers still write before generic acknowledgement, so their stale-result publication and atomic follow-up migration remain open gates in the [approved plan](../scoracle-wiki/progress_docs/scoracle-backend/2026-09-19_backend-modernization-plan.md).
...
Scout still creates from the established statistical, personnel, availability, and sourced-memory contract in `junctions/scout`; this bounded slice did not rewrite its prompt or evaluation path. The application adapter now owns queue and publication effects. A no-stats result atomically publishes its NULL-body marker and the normal Momentum/Oracle obligation. An unchanged stats result publishes no row and records only the Oracle check, preserving the previous debounce behavior. Transfer, availability, and packet-triggered reruns still bypass that stats-only debounce.
...
| `rust/src/application/` | Explicit application adapters; Analyst, Influencer, and Scout publication/work coordination. |
...
Studio's Analyst and Influencer tests plus Scout's application lifecycle tests use fake model, publication, and follow-up adapters and require no service credentials. Model-quality evaluations and live database integration checks are separate gates. See [rust/README.md](rust/README.md).
