...
- **Studio core and migrated seats:** `rust/src/studio/` owns the model contract, generation/provenance types, shared character form, and Analyst, Influencer, and Scout creation. Their assignments run without Postgres, DuckDB, queues, or concrete model-host knowledge. Scout now has a small subject/material/session contract rather than a durable entity key inside its character API.
- **Application adapters:** `rust/src/application/analyst.rs`, `influencer.rs`, and `scout.rs` own concrete retrieval, memory preparation, model selection, work policy, and claim-aware publication. Scout's PostgreSQL evidence reads and material renderers are explicit child adapters. `rust/src/application/outbox.rs` durably reconciles their concrete Momentum/Oracle obligations. `runtime/worker.rs` and `runtime/work.rs` retain the transitional queue lifecycle.
...
| `rust/src/studio/` | In-house harness, Analyst/Influencer/Scout creation, shared form, and generation contracts. |
| `rust/src/application/` | Explicit retrieval, routing, publication, and work adapters for migrated seats. |
| `rust/src/junctions/` | Six remaining model-facing seats and their transitional adapters. |
...
Studio's Analyst, Influencer, and Scout creation tests plus application lifecycle tests require no service credentials. Model-quality evaluations and live database integration checks are separate gates. See [rust/README.md](rust/README.md).
