...
- **Studio core and migrated seats:** `rust/src/studio/` owns the model contract, generation/provenance types, shared character form, and Analyst, Influencer, Scout, and Journalist creation. Their assignments run without Postgres, DuckDB, queues, or concrete model-host knowledge. Durable entity keys and trigger policy remain outside character APIs.
- **Application adapters:** `rust/src/application/analyst.rs`, `influencer.rs`, `scout.rs`, and `journalist.rs` own concrete retrieval, memory preparation, model selection, work policy, and claim-aware publication. Scout's PostgreSQL evidence reads and material renderers are explicit child adapters. Journalist's adapter owns packet retrieval and atomic multi-row/marker publication plus storyline progression. `rust/src/application/outbox.rs` durably reconciles their concrete Momentum/Oracle obligations. `runtime/worker.rs` and `runtime/work.rs` retain the transitional queue lifecycle.
- **Other character work:** five seats still use `junctions/` and the database-bearing `runtime/harness.rs` compatibility context. Shared extraction delegates to Studio. This context is migration scaffolding; it is not a second permanent harness.
...
- **Durability:** migration 256 and `runtime/work.rs` give each running lease a UUID claim token plus its captured input revision. Migrations 257–260 make Influencer, Analyst, Scout, and Journalist claim-aware publishers: after inference, each short transaction locks the exact lease, writes any product with required provenance, records only its durable follow-up obligation, and deletes the claim. A stale or superseded execution publishes nothing. Product-bearing Vibe and Rating completions offer Momentum before checking Oracle; Momentum, debounced Rating, and Narratives completions need only the Oracle barrier. Analyst `NoMaterial`, Scout debounce, and Journalist debounce write no product while still committing their barrier obligation and completion; Scout's no-stats marker and Journalist's no-corpus/called-empty marker remain real products. Journalist also commits every chapter and its storyline progression in that same transaction. Diagnostic ledger writes remain explicitly best-effort after commit. Other handlers still write before generic acknowledgement, so their stale-result publication and atomic follow-up migration remain open gates in the [approved plan](../scoracle-wiki/progress_docs/scoracle-backend/2026-09-19_backend-modernization-plan.md).
...
| `rust/src/studio/` | In-house harness, Analyst/Influencer/Scout/Journalist creation, shared form, and generation contracts. |
| `rust/src/application/` | Explicit retrieval, routing, publication, and work adapters for migrated seats. |
| `rust/src/junctions/` | Five remaining model-facing seats and their transitional adapters. |
...
Studio's Analyst, Influencer, Scout, and Journalist creation tests plus application lifecycle tests require no service credentials. Model-quality evaluations and live database integration checks are separate gates. See [rust/README.md](rust/README.md).
