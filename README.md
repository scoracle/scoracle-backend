...
- **Durability:** migration 256 and `runtime/work.rs` now give each running lease a UUID claim token plus its captured input revision. Complete, fail, defer, and release require that identity; a newer desired revision or reclaimed lease survives an older execution, and only an applied completion may offer the Oracle follow-up. Product writes still happen inside handlers before acknowledgement, so stale-result publication, transactional product/provenance/follow-up, outbox recovery, and packet fan-out migration remain separate gates in the [approved plan](../scoracle-wiki/progress_docs/scoracle-backend/2026-09-19_backend-modernization-plan.md).
