...
- **Application adapters:** `rust/src/application/influencer.rs` owns Influencer retrieval, memory preparation, early debounce, and claim-aware publication. `rust/src/application/outbox.rs` durably reconciles its Momentum offer and Oracle barrier after publication. `rust/src/junctions/analyst/` prepares evidence using existing Postgres loaders and publishes to `momentum_summaries` plus the existing best-effort ledger. `runtime/route.rs` selects/governs model transports. `runtime/worker.rs` and `runtime/work.rs` retain the current queue lifecycle.
...
- **Durability:** migration 256 and `runtime/work.rs` give each running lease a UUID claim token plus its captured input revision. Migration 257 makes Influencer the first claim-aware publisher: after inference, one short transaction locks that exact lease, writes `vibe_scores` with required provenance, records a durable `vibe_completed` outbox event, and deletes the claim. A stale or superseded Influencer publishes nothing. The worker retries the event's idempotent Momentum offer and Oracle barrier. Its diagnostic ledger remains explicitly best-effort after commit. Other handlers still write before generic acknowledgement, so their stale-result publication and atomic follow-up migration remain open gates in the [approved plan](../scoracle-wiki/progress_docs/scoracle-backend/2026-09-19_backend-modernization-plan.md).
