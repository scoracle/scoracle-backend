...
- **Studio core and migrated seats:** `rust/src/studio/` owns the model contract, generation/provenance types, shared character form, and Analyst, Influencer, Scout, Journalist, Insider, Oracle, and Editor creation. Their assignments run without Postgres, DuckDB, queues, or concrete model-host knowledge. Insider owns pair verdict, identity-adjudication, and wrap contracts; Oracle receives five typed cards and interprets complete, partial, or empty spreads inside Studio.
- **Application adapters:** `rust/src/application/` owns concrete retrieval, memory preparation, model selection, work policy, and claim-aware publication for the seven migrated seats. Insider's adapter owns transfer retrieval, partial pair progress, identity effects, routing, and fenced publication; Oracle's owns pillar readiness/enqueue policy and terminal crown publication. `rust/src/application/outbox.rs` durably reconciles concrete Momentum/Oracle obligations, including transfer-pair fanout.
- **Other character work:** Investigator and Graph still use `junctions/` and the database-bearing `runtime/harness.rs` compatibility context. Shared extraction delegates to Studio. This context is migration scaffolding; it is not a second permanent harness.
...
| `rust/src/studio/` | In-house harness, seven migrated seat assignments including Editor, shared form, and generation contracts. |
| `rust/src/application/` | Explicit retrieval, routing, publication, and work adapters for migrated seats. |
| `rust/src/junctions/` | Two remaining model-facing seats and their transitional adapters. |
...
Studio's seven migrated seat creation tests plus application lifecycle tests require no service credentials. Model-quality evaluations and live database integration checks are separate gates. See [rust/README.md](rust/README.md).
