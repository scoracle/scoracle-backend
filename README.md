...
- **Application runtime:** `application/worker.rs` owns queue dispatch, maintenance, and recovery. Bound `WorkHandler`s accept only an exact claim and return a durable disposition; each adapter owns its publication transaction. Storage-only helpers take a pool, while `application/models.rs` holds only model routes and limits. The legacy database-bearing harness, junctions, composition compatibility exports, and generic worker completion are removed.
- **Analytics:** the incorporated `cleanup/character-prompts` work (`ea20950`, including `6ded51e`) supplies `go/internal/analytics/`, Postgres/DuckDB implementations for rating trajectories and bundles, the snapshot CLI, and cohort context consumed by memories. The legacy trajectory/bundle path can attach Postgres read-only; Postgres remains the default engine. The cohort CLI now defaults to read-only shadow comparison over a retained, bounded MVCC export, with explicit fenced whole-cohort publication behind migration 262. Local recovery and live NBA shadow parity pass; production publication and cutover remain gated. See [acceptance operations](run_docs/RECOVERY_ANALYTICS_ACCEPTANCE.md). Reuse this engine and its parity tests rather than building a competing implementation.
- **Momentum maintenance:** Go now commits SQL scores, their concurrent serving-projection refresh, and exact dirty-marker acknowledgement together. Failure preserves retryable work and the prior projection; newer markers survive. This local recovery fix retains the existing SQL producer and needs destination-host acceptance before deployment.
- **Durability:** migration 256 and `runtime/work.rs` give each running lease a UUID claim token plus its captured input revision. Migrations 257–261 make Influencer, Analyst, Scout, Journalist, and Insider claim-aware publishers with narrow durable follow-up obligations. Insider preserves partial progress while each pair and identity effect is fenced by the exact team lease; transfer products atomically record per-target Oracle barriers and final completion records the team barrier. Oracle uses the same exact-claim boundary without a new outbox obligation because it is terminal. A stale or superseded execution publishes nothing. Diagnostic ledger writes remain explicitly best-effort after commit. All nine seats now have a local publication boundary; live recovery and cutover acceptance remain; see the [approved plan](../scoracle-wiki/progress_docs/scoracle-backend/2026-09-19_backend-modernization-plan.md).
...
| `rust/src/studio/` | In-house harness, all nine migrated seat assignments, shared form, and generation contracts. |
...
Studio's nine migrated seat creation tests plus application lifecycle tests require no service credentials. Model-quality evaluations and live database integration checks are separate gates. See [rust/README.md](rust/README.md).
