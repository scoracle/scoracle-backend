...
- **Remaining runtime cleanup:** all nine model-facing seats now create in Studio. The database-bearing `runtime/harness.rs`, worker interface, and `junctions/mod.rs` compatibility re-exports remain for the separately scheduled cleanup session.
...
| `rust/src/junctions/` | Only compatibility re-exports remain; remove in the runtime/plumbing session. |
