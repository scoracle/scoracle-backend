...
> simplification chapter (`docs/progress_docs/2026-08-20_single-box-consolidation.md`).
...
**BASELINES — the first full set ever taken on the PRODUCTION engine (oMLX/8B, quiet, two byte-stable runs each), and the engine mattered twice:**

| seat | baseline (grown gate) | post-edit ×2 | note |
|---|---|---|---|
| vibe v17→v18 | 53/53 | **53/53** | diet only (packets 4→2) |
| rating s17→s18 | 97/99 | **91/95 → see addendum** | denominator moved: peak axes retired (−), section labels/floors/invariant added (+) |
| narratives n18→n19 | 109/110 | **110/110** | ⭐ perfect — edition budget AND card_score hold |
| momentum s14→s15 | 77/91 | **87/91** | the 10 no_product_names reds at baseline WERE the s14 contract; the inversion took cleanly |
| oracle or8→or9 | 87/108 (78/98 pre-invariants) | **108/110** | parse fix + blindness + plain-text; residual = the single-peer rule on ONE disagreement fixture (peers=4 ×2 runs) — the QUEUED register-pass item, now the crown's only defect class |

**⚠ THE CROWN'S oMLX PARSE BUG — found by the baseline, fixed, production-confirmed.** The or8 unparseable fixture returned a COMPLETE reply (finish_reason stop): unconstrained on oMLX (response_format withheld there — the tekken finding), the 8B fences the JSON and writes paragraph breaks as LITERAL newlines inside the JSON string — illegal JSON, both parse paths reject, the row fails. **Production had exactly 18 failed sigil rows with this error** since the flip. Fix: `parse_crown_reply`'s brace-salvage folds `\n\r\t`→space inside the span (semantics-preserving; the reading normalizer collapses whitespace anyway). The fixture went 0/16 → 18/18; the 18 rows clear on requeue.

**THE HARNESS GREW FIRST:** diacritic folding in `contains_ci` (Sørensen dead; `fold_quotes`→`fold_for_match`); the shared case-sensitive `PRODUCT_NAME_BANS`/`no_product_names` per-reply invariant on rating+momentum+oracle (case-sensitive because lowercase "peak" is honest English); `reading_plain_text` on the crown (or8 served `*there*` italics through a GREEN gate — a rule measured by nothing is advice); `prose_includes_any` pipe-delimited any-of groups ("name the signal" now means the sport's varying words).

**⛔ THE GENERATOR-DRIFT TRAP — caught twice in one evening.** The momentum generator was missing the D-T51/52 gate growth that lived only in on-disk JSON (incl. BOTH hand-authored direction fixtures — a regen would have silently dropped them); caught by inspection and adopted into the generator. Then the vibe/rating/narratives regens DID silently shrink their gates (vibe 53→17 axes) — caught against baseline denominators, restored from git-HEAD expects, and ported into ALL generators (shared `vibe_gate`/`rating_gate` helpers). Corollary: `Expect` ignores unknown fields, so a generator's stale inert key (`prose_includes_any` before the axis existed) became ACTIVE with foreign semantics the moment the axis landed — field now skip-serializing, stale keys gone. **A regen is faithful by construction now; the drift class is structurally closed.**

**THE FIVE BUMPS (one per voice, full rationale in each prompt.rs version comment):** Scout s18 — divined_peak CODE-OWNED (RatingReady carries the deterministic label; copy-flake class dead), prompt+decision-card+memory-card fully de-PEAKed (the s13 lesson as design: the input stops shouting first; mig 164's card descrubbed in Rust), two-rule SHIPS block (plain text + product names). Analyst s15 — rule 1 inverted to the sport's words ("the form/the tape", "the mood/emotion around the club"), input labels renamed (FORM/MOOD TREND, `z_trajectory_label` descrubbed at its scout/mod.rs source — it feeds the crown's card too), both worked examples rewritten. Oracle or9 — blind to memories (prior-read + relational-memory gone from prompt AND stage; `load_prior_read` deleted, zero other callers), card labels descrubbed, system diet ~1,806→~1,050 tok (mid-list duplicates of the SHIPS rules deleted, unique clauses merged in — the s12 treatment); CROWN_CARD_BODY_CAP deliberately stays 700. Vibe v18 — MAX_VIBE_PACKETS 4→2 (the census's fattest seat; packets ~2k tok each are the guard-tripping tail). Journalist n19 — edition budget + card_score calibration promoted to a SHIPS block grounded in the CARD (Scott's card brief); the 8B's 52-sentence edition and card_score-1 were one defect (verdict ≠ filing).

**Also closed from the owed list:** momentum markdown-prose recurrence check reads **0 rows** (the D-T52 fix held); the audit of the junctions layer is delivered (13 DEAD, 8 judgment calls, 12 misleading docs — Scott's cleanup queue); the 8B's fabricated-statistics class ("the 28th percentile is a 72% chance") is RECORDED as un-gated — present at s17 too, candidate for its own pass.

**ADDENDUM — the rating notation-copy promotion (same evening):** the post-edit gate read the `" · "` card-notation copy at 3× (baseline 1×) — the mid-list-rule-ignored pattern, now measured. The ban moved into SHIPS rule 1 (typography: asterisks AND the card's " · " separator), fixtures re-frozen, re-gated in the same window: **94/95 ×2 byte-stable — all three notation copies gone; the promotion treatment works for the FOURTH consecutive time (s9 Markdown, s12 naming/closers, or8 field words, s18 notation)**. The one remaining rating residual is the documented `giveaway` mention-to-dismiss honesty gap (D-T51). **FINAL GATE TABLE: vibe 53/53 · rating 94/95 · narratives 110/110 · momentum 87/91 · oracle 108/110 — 452/464 (97.4%), every residual a named, documented class.**
