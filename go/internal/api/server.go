...
func NewRouter(pool *pgxpool.Pool, appCache *cache.Cache, cfg *config.Config) *chi.Mux {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/{sport:nba|nfl|football}", func(r chi.Router) {
			...
			// generated momentum product, /vibe the Influencer's emotional read, and
			// /sigil the final crown synthesis.
			r.Get("/{entityType:player|team}/{id}/stats", h.GetEntityStats)
			r.Get("/{entityType:player|team}/{id}/rating", h.GetEntityRating)
			r.Get("/{entityType:player|team}/{id}/vibe", h.GetEntityVibe)
			...
			r.Get("/{entityType:player|team}/{id}/articulator/{kind:p1|p2|p3|p4|p5|p6|p7|p8}", h.GetArticulatorSlice)
			...
			// reporting week, merged — the card contract's index (2026-08-24;
			// weeks are the sport's own calendar since mig 237).
			r.Get("/{entityType:player|team}/{id}/headlines", h.GetEntityHeadlines)
			// The reporting calendar itself (mig 237): the week-nav's data source.
			r.Get("/weeks", h.GetSportWeeks)
