...
@click.group()
def cli():
    """Scoracle NFL seeder CLI."""
    pass


@cli.command()
@click.option("--team-id", default=1, help="Team ID to test (default: 1)")
...
async def _test_team(team_id: int):
    ...
    try:
        async with BallDontLieNFL(api_key) as client:
            seeder = NFLSeeder(conn, client)
...
@cli.command()
@click.option("--player-id", default=1, help="Player ID to test")
...
@click.option("--postseason", is_flag=True, help="Seed postseason stats")
def seed_player_stats(season: int, postseason: bool):
    """Seed player stats for a season."""
    asyncio.run(_seed_player_stats(season, postseason))


async def _seed_player_stats(season: int, postseason: bool):
    db_url = get_database_url()
    api_key = get_api_key()
    ...
    try:
        async with BallDontLieNFL(api_key) as client:
            seeder = NFLSeeder(conn, client)
            result = await seeder.seed_player_stats(season, postseason)
...
@click.argument("season", type=int)
@click.option("--postseason", is_flag=True, help="Seed postseason stats")
def seed_team_stats(season: int, postseason: bool):
    """Seed team stats for a season."""
    asyncio.run(_seed_team_stats(season, postseason))


async def _seed_team_stats(season: int, postseason: bool):
    db_url = get_database_url()
    api_key = get_api_key()
    ...
    try:
        async with BallDontLieNFL(api_key) as client:
            seeder = NFLSeeder(conn, client)
            result = await seeder.seed_team_stats(season, postseason)
...
@click.argument("season", type=int)
@click.option("--postseason", is_flag=True, help="Seed postseason stats")
def seed_all(season: int, postseason: bool):
    """Seed all NFL data for a season (teams, players, stats)."""
    asyncio.run(_seed_all(season, postseason))


async def _seed_all(season: int, postseason: bool):
    db_url = get_database_url()
    api_key = get_api_key()
    
    ps_label = "postseason" if postseason else "regular"
    click.echo(f"Starting full NFL seed for {season} ({ps_label})")
    
    conn = await asyncpg.connect(db_url)
    try:
        async with BallDontLieNFL(api_key) as client:
            seeder = NFLSeeder(conn, client)
            result = await seeder.seed_all(season, postseason)
...
@click.option("--postseason", is_flag=True, help="Seed postseason stats")
def backfill(start_season: int, end_season: int, postseason: bool):
    """Backfill NFL data for multiple seasons."""
    asyncio.run(_backfill(start_season, end_season, postseason))


async def _backfill(start_season: int, end_season: int, postseason: bool):
    db_url = get_database_url()
    api_key = get_api_key()
    
    ps_label = "postseason" if postseason else "regular"
    click.echo(f"Starting NFL backfill from {start_season} to {end_season} ({ps_label})")
    ...
    try:
        async with BallDontLieNFL(api_key) as client:
            ...
            for season in range(start_season, end_season + 1):
                ...
                ps_result = await seeder.seed_player_stats(season, postseason)
                ...
                ts_result = await seeder.seed_team_stats(season, postseason)
