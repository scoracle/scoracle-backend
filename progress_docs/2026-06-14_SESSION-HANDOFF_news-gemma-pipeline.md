# 2026-06-14 — SESSION HANDOFF: News→Gemma pipeline (staged build)

Capstone for a long build session. Read this + memory `news-gemma-pipeline` +
`gemma-two-rail-narratives` + vault `Plan - News pipeline integration.md` to resume. Per-task detail
...
- `7202797` task 3 — **async scrub** as a `news_scrub` maintenance ticker (30m/15; auto-vet primaries +
  Gemma-disambiguate secondaries). **LIVE** — coverage building (106k→~45k unscrubbed).
...
## LOCKED DESIGN — staged Gemma pipeline (each stage enriches the next; sentiment LAST)
```
...
- Sourcing-from-scrubbed is wired (084 + task 4); the Gemma `is_rumor` vet already clears same-name noise.
...
6. **Task 8 — stats-rail fast-follow** (Gemma stat-profile summaries + Special card; reuses the primitives).
...
5. Regenerate transfers off the vetted corpus: `cmd/transfer -mode corpus` (heavy Gemma; staggered).
...
- DEFERRED: transfer Gemma vet→determine promotion (Roadmap 1a); narrative genericization prompt-tuning
  (Gemma sometimes says "a super striker" when the source headline was clickbait-vague).
