...
(does it agree with / beat Gemma on labeled data?) instead of byte-parity.
...
fused Gemma call** (is_rumor + subject + stage + summary in a single verdict), so a transfers port
...
foundation every derivation sits on) by stuffing an identity card into a Gemma prompt and hoping.
The better process is cheap **CPU embeddings (candle)** that pre-filter before the expensive GPU
call — more accurate AND fewer Gemma calls. That is the value the Rust layer uniquely adds.
...
- **Resolve experiment** (`rust/src/bin/resolve_experiment.rs`) — the measured de-risk. Embeds
  article↔identity-card for Gemma's labeled scrub verdicts and scores cosine separation.
  ...
  *secondary* player links. Banding (reweighted to the real 4920:879 base rate): the conservative
  band **0.60/0.75 → ~55% of Gemma calls saved, 97% auto-keep precision, ~0% recall lost**.
- **Hybrid Resolve gate — REAL** (`rust/src/resolve.rs`). `resolve_set` (scrub shape) + `resolve_one`
  ...
  confident tails (no model call) → Gemma adjudicates only the ambiguous middle → **fail-closed**
...
- **Resolve eval** (`rust/src/bin/resolve_eval.rs`) — runs the REAL `resolve_set` end-to-end over a
  labeled article sample, reporting agreement vs Gemma + the live auto-decide/Gemma split.
  **Result (40 articles / 43 candidates): accuracy 0.930, precision 0.946, recall 0.972, F1 0.959
  vs Gemma; 58% of candidates auto-decided without a Gemma call → 58% GPU saved in situ** (matching
...
- **Embeddings on the CPU, Gemma on the GPU — no contention.** Archbox (i7-7700, AVX2+FMA, 31 GiB,
  GTX 1070 Ti 8 GB) is a perfect fit: embeddings run on the otherwise-idle CPU (~tens of ms/doc,
  *noise* next to a 2–8 s Gemma call), and the pre-filter makes net GPU load go DOWN.
- **Resolve is a HYBRID, not a Gemma replacement.** Embeddings catch *topical* mismatch (different-
  club impostor → low cosine); they miss *on-topic* false positives (right sport/club, wrong subject
  — e.g. Tua Tagovailoa 0.832), which is exactly the band Gemma keeps adjudicating.
...
cargo run --release --bin resolve_experiment     # the AUC/banding signal (embedding-only, no Gemma)
...
35 + 1 ignored (real-model) · experiment **AUC 0.880** (2,379 links) · eval **F1 0.959 vs Gemma,
