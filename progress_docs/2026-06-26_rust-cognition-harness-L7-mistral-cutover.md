# L7 — One model, not two: the Mistral 7B cutover (off the oversized Gemma)
...
L7 set out to run two role-specialized models (Mistral news + Gemma stats) on the 1070 Ti with a
batch-by-model scheduler. Reconnaissance + a measured bake-off **collapsed that premise into something
simpler and bigger**: replace the oversized multimodal `gemma4:e4b` with a single fitting text model,
**`mistral:7b`**, for *everything*. Cutover is **live on archbox** (~19 tok/s, ~2× the old Gemma).

## The reconnaissance that changed the plan

- The "2025 Gemma stats backlog" the handoff targeted is **already computed** (rating/sigil/stat_summaries
...
- `gemma4:e4b` is an **8B multimodal** model (Q4_K_M, carries vision+audio weights we never use). `ollama ps`
...
|---|---|---|
| gemma4:e4b (old) | 10 GB → **67% CPU** | verbose (500–800 tok, ignores "two lines"), drama-skewed, sometimes wrong ("Mayfield = trade chip") |
| gemma2:2b | 2.6 GB → 100% GPU | formulaic, narrow scores, **over-commits** ("Giannis joins Miami" as fact) |
| **mistral:7b** | **5.2 GB → 92% GPU** | concise, specific, instruction-compliant — **the pick** |
| gemma2:9b | 8.1 GB → slight spill | most measured/nuanced; borderline fit |
...
**follows instructions** (Gemma ignored format constraints; the defensive prompt "clamps" were Gemma-tax).
...
- **`gemma4:e4b` removed** (`ollama rm`) — this both streamlines the stack AND clears the wedged-in-VRAM
  model that had been forcing Mistral onto the CPU. Bake-off scratch (gemma2:9b/2b, nemo) also removed.
- **Result: ~19 tok/s** (vs gemma4's 9.9 clean baseline; ~2×), Mistral sole-resident at 92% GPU.
...
- **Streamlined stack (user-confirmed):** the only LLM is `mistral:7b` (Ollama / GPU diviner); the only
  other model is **BGE-small** (candle / CPU) — the front-of-funnel scrub *sieve*. `gemma:latest`, the last
...
Mistral follows instructions, so the prompts can shed their Gemma-era **defensive clamps** (rigid format
