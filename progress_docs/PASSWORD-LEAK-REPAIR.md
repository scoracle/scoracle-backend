...
| **Residual — GitHub PR refs** | ✅ **NEUTRALIZED 2026-08-01.** `refs/pull/1/head` and `refs/pull/2/head` are read-only hidden refs GitHub refuses to let anyone rewrite (`deny updating a hidden ref`), and they still carry 4 literals. All four are now **dead values**: Neon pw (projects deleted), `API_SPORTS_KEY` (revoked), Twitter bearer (revoked), old Postgres pw (rotated). The repo is private on top of that. A GitHub Support GC request is optional tidiness now, not a fix. |
| **Residual — public legacy repo** | ✅ **DELETED 2026-08-01.** `albapepper/Scoracle` (PUBLIC, `API_SPORTS_KEY` ×7 commits), superseded by `scoracle/scoracle-backend`, is gone from GitHub; its orphaned 805 MB local checkout on archbox was removed too. Its one secret was already revoked, so this was hygiene, not exposure. Mirror backup retained: `archbox:~/scoracle-PREPURGE-2026-08-01/Scoracle-github.git` (455 commits, 59 refs, fsck clean). |
