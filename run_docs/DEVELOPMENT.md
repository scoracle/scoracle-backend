...
A Studio character consumes values describing its assignment, not rows or another character's storage representation. Analyst, Influencer, Scout, and Journalist creation live in `studio/`; their production preparation and publication live in explicit `application/` adapters. Scout and Journalist deliberately separate their durable application requests from their small `Subject` and prepared `Assignment` contracts. None has a compatibility junction. `runtime/harness.rs` is a transitional application context for unmigrated seats, not a second permanent harness.
...
One producer owns each live output during cutover. Migration 256 establishes queue acknowledgement ownership: a running item has a unique claim token and captured input revision, and every complete/fail/defer/release must match both. Migrations 257–260 apply the publication pattern to Influencer, Analyst, Scout, and Journalist: infer without a transaction, then lock the exact claim and commit any product plus required provenance, the seat's narrow durable follow-up intent, and claim deletion together. A superseded execution publishes nothing. Analyst `NoMaterial`, Scout debounce, and Journalist debounce commit no product but still record the required Oracle-barrier obligation. Scout's no-stats marker follows the normal Momentum path; Journalist's no-corpus or called-empty edition writes one marker. Journalist also commits all chapter rows and storyline progression atomically. Optional diagnostic ledger writes remain best-effort after commit and are not proof of durability. New claim-aware handlers should override `StageHandler::handle_claimed`; legacy handlers keep the generic worker completion path until migrated. Use the approved wiki modernization plan for the remaining seats and gates.
...
- `rust/src/junctions/` - five remaining model-facing seats and transitional adapters.
