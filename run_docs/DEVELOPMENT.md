...
One producer owns each live output during cutover. Migration 256 establishes queue acknowledgement ownership: a running item has a unique claim token and captured input revision, and every complete/fail/defer/release must match both. Migrations 257–259 apply the publication pattern to Influencer, Analyst, and Scout: infer without a transaction, then lock the exact claim and commit any product plus required provenance, the seat's narrow durable follow-up intent, and claim deletion together. A superseded execution publishes nothing. Analyst `NoMaterial` and Scout debounce commit no product but still record the required Oracle-barrier obligation; Scout's no-stats marker is a product and follows the normal Momentum path. Optional diagnostic ledger writes remain best-effort after commit and are not proof of durability. New claim-aware handlers should override `StageHandler::handle_claimed`; legacy handlers keep the generic worker completion path until migrated. Use the approved wiki modernization plan for the remaining seats and gates.
