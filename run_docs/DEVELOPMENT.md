...
A Studio character consumes values describing its assignment, not rows or another character's storage representation. Analyst, Influencer, Scout, Journalist, Insider, and Oracle creation live in `studio/`; their production preparation and publication live in explicit `application/` adapters. Insider receives prepared transfer pairs, evidence, verification material, and prior reads; Oracle receives five typed cards, never product-table rows or raw articles. None has a compatibility junction. `runtime/harness.rs` is a transitional application context for unmigrated seats, not a second permanent harness.
...
- `rust/src/junctions/` - three remaining model-facing seats and transitional adapters.
