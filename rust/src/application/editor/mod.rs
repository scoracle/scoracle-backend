...
async fn commit_claimed(pool: &PgPool, item: &Item, prepared: &Prepared) -> Result<HandleOutcome> {
    ...
    match prepared {
        ...
        Prepared::Terminal {
            ...
            persist_terminal(&mut tx, item, status, url, domain, words, error.as_deref()).await?;
            ...
            let status = match &extracted.value {
                Some(read) => {
                    let resolved = if read.relevant {
                        resolve::resolve_names(&mut tx, &item.sport, &read.names).await?
                    } else {
                    ...
                    .await?;
                    nominate::nominate_fixture_from_result(
                        &mut tx,
                        &item.sport,
                        item.entity_id,
                        read,
                    ...
                    .await?;
                    if read.relevant {
                        harvest_team_links(&mut tx, item.entity_id, &item.sport, read).await?;
                    }
                    enqueue_graph_for_article(&mut tx, item.entity_id, &item.sport).await?;
                    status
                }
                None => {
                    persist_terminal(
