...
pub fn build_momentum_prompt(
    ...
    match rating {
        Some(r) => {
            ...
            // The trajectory label carries the Scout's internal vocabulary ("Composite and PEAK
            // z-scores trending up over recent games"). s13 banned those words in the READ and s14
            // additionally flagged the label here as internal — BOTH made it worse (109 checks:
            // 101 at s13, 98 at s14), because naming the vocabulary next to the data raises its
            // salience instead of suppressing it. The label renders plainly again; see the version
            // note for why the ban itself was withdrawn.
            let label = if !r.peak_trajectory_label.trim().is_empty() {
                &r.peak_trajectory_label
            } else {
                &r.peak_trajectory
            };
            b.push_str(&format!("PEAK trajectory: {label}\n"));
        }
