...
READ: <up to eight sentences — write only as many as the numbers support; every quantity in words, never a digit>
...
pub fn build_momentum_prompt(
    ...
    match rating {
        Some(r) => {
            // s16 (the PEAK retirement): the divined top-skill line is gone — the scouting
            // read below names the standouts itself now.
            ...
            let label = if !r.rating_trajectory_label.trim().is_empty() {
    ...
    // The decided facts the model narrates (never decides) — the omen pattern. BOTH
    // arrive as WORDS (s18): the s13/s15 postmortems showed a ban cannot beat a phrase
    // sitting in the model's own input, and until s18 this line handed the READ a
    // figure and the words "steady band" in the same breath it was told to emit
    // neither — digits_in_read was 65% of momentum generations by 2026-08-22. The
    // strength ladder mirrors momentum_conviction_from_score exactly, so the prose and
    // the persisted ±5 conviction can never tell two stories.
