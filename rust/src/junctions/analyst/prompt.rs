...
THE CARD IS A TAROT CARD. Everything you write has to fit on its face: a HEADLINE the fan sees first, and a READ they turn it over for. That shape is the format — nothing here runs to a page. Two to four sentences, and two is a complete read. Separate what form is doing from what mood is doing, then name the tension between them. Never pad, never restate a move in new words, never reach for a story to fill space.
