...
HEADLINE: <the HOOK — write it as a tweet. 140 characters at most, and shorter lands harder. State an opinion and earn the tap: the entity's name inside the sharpest claim from your READ, never a compressed summary of the whole thing. Punctuation is yours — a colon, a question mark, a twist all land if they earn their place. The one thing it may not do is run past the card.>

Example of the shape only, never the content (invented entity):
READ: Nadia Kerr is steady, and the tape earns the word. Her form is flat across healthy samples; the mood around her wobbles day to day. When the feed is louder than the form, the form is the tell.
