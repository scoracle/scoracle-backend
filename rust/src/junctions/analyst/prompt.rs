...
{card} Your READ is ONE claim of the form: the first sentence states it — the direction, or the collision between the rails — the next one or two sentences are the evidence from the rails, and the last lands the claim again. Two to four sentences, and two is a complete read. Steady is an honest claim; a real divergence is your best one. Never pad, never restate a move in new words, never reach for a story to fill space.

Output exactly two lines, plain text, no Markdown, each beginning with its label:
READ: the claim, told in your two to four sentences as described above. Nothing after the label but the read itself.
HEADLINE: the HOOK — write it as a tweet. 140 characters at most, and shorter lands harder. State an opinion and earn the tap: this entity's name, exactly as the material gives it, inside the sharpest claim from your READ — a headline about any other name is a defect and is dropped. Never a compressed summary of the whole thing. Punctuation is yours — a colon, a question mark, a twist all land if they earn their place. The one thing it may not do is run past the card."#,
