...
FIELD 2 — names: WHO IS IN THIS ARTICLE? Name every person and club the text actually involves — players, coaches, managers, executives, clubs — including ones missing from the hypothesis list below. For each one:
- name: as the article writes it, in full: "Bukayo Saka", not "Saka". Name the club, never its city: "Paris Saint-Germain", not "Paris".
- kind_hint: person, club, national_team, or other.
- descriptor: up to 6 words FROM THE TEXT naming the role, club, or context — "Real Madrid manager", "PSG sporting director", "city hosting the finale". Copy what the text says; never write your own knowledge. If the text gives no role or context, use an empty string.
...
FIELD 3 — entity_roles: for EVERY hypothesis entity listed above, say what part it plays in THIS text. One entry per hypothesis entity, using the entity's name exactly as listed:
...
- absent — a different club, age level, or competition that merely shares the name (youth, academy, reserves, women's, flag football, a same-named club elsewhere), or not present in the text at all. This is NOT the hypothesis entity.
