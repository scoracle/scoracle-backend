...
YOUR MATERIAL IS THE DESK'S STORIES AND NOTHING ELSE. A story that is not really about this entity — someone else's match, someone else's saga your entity brushes against — gets no feeling and no sentence: skip it whole. You read only what the desk compiled; there is no other input to reach for.
...
VIBE: <the body in THE FORM — paragraphs separated by blank lines>"#,
        ...
        headline = crate::junctions::form::headline_contract("HOOK", "the sharpest feeling in your VIBE")
    )
});
...
pub fn build_sentiment_prompt(
    ...
    // v26, THE DE-DUPE (2026-09-06, Scott: "there's a huge overlap between the narratives
    // and vibes output… the two most influential, pillar voices then become Scout and
    // Influencer — logic vs emotion"): the Journalist's narratives block is GONE from this
    // prompt. Since the packet rail (7.6) she read the same packets he reads PLUS his
    // write-up of them — double-fed the same stories, so her card duplicated his. The
    // packets are her material now, whole and alone; the Journalist's retirement plan is
    // PLAN-journalist-retirement.md. The `narratives` param stays for the loaders/hash
    // (transitional — the plan's stage 2 removes it end to end).
    let _ = narratives;

    // v27 (the feeds matrix, Scott 2026-09-06: "This junction should be receiving just the
    // info from the editor… only the analyst (scout+influencer feeds) and oracle (all five
    // feeds) should receive data from the other characters"): the Insider-derived transfer
    // temperature is GONE — the last cross-character feed on this seat. An emotionally live
    // move reaches her through the desk's stories, which carry the mood and the names; the
    // v20 SCORE anchors that referenced the wire aggregate were reworded in the same pass.
    // The `heat` param survives transitionally for the loaders/hash/empty-check (the same
    // treatment as `narratives`; the cascade cleanup removes both end to end).
    let _ = heat;
