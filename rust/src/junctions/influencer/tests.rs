...
#[test]
fn rejects_non_current_vibe_order() {
    assert!(parse_vibe_reply("SCORE: 60\nVIBE: line one\nHOOK: The title").is_err());
    assert!(parse_vibe_reply("HOOK: The title\nSCORE: 60\nVIBE: line one").is_err());
    assert!(parse_vibe_reply("**SCORE:** 60\nHOOK: The title\nVIBE: line one").is_err());
}
...
#[test]
fn errors_without_score_line() {
    assert!(parse_vibe_reply("the vibe is about 64 today").is_err());
}
...
#[test]
fn vibe_parser_requires_the_current_hook_line() {
    assert!(VibeParser
        .parse("SCORE: 73\nVIBE: Quietly surging into the playoff race.")
        .is_err());
}
