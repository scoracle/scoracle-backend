...
pub const INSIDER_SCORE_PROMPT_VERSION: &str = "is7"; // is7, THE UNIFORM STRUCTURE (2026-09-06, Scott: "a uniform mechanical structure across all characters — the unique aspect of each junction should just be the individual voice"): composes STORY_FORM, card_face and form::headline_contract; the hand-rolled "form told tight" description and the eight-sentence LENGTH paragraph retire — the form's claims-per-card shape supersedes the sentence allowance (the v24 precedent). The claims-definition ("your claims are THE BOARD") and the padding-is-misinformation doctrine are the seat's contribution. Part of the deliberate six-voice bump; keys the wrap's own input_hash, so the bump self-backfills one wrap per rumor-active entity. // is6, THE FORM + THE WAVE (2026-08-25): composes CLAIM_SELECTION + WIRE_COPY from junctions::form; the READ is named as the form told tight (board claim → per-call evidence sentences → quickening-or-settling close — the shape the contract always described). Part of the deliberate five-voice bump for the granite+form regen wave (see momentum-s21's note for the full rationale): the articulator corpus gate requires is6+ AND model_version=granite4.2:3b. // is5, THE HEADLINE (2026-08-24, mig 232 — the uniform score+headline+body card contract): the wrap gains the required entity-level `headline` — the card's tweet hook (140 chars, guards::settle_title at parse), the one-line read of the WHOLE wire. A quiet wire's honest hook is explicitly blessed in the contract (Scott: "if there's been no transfer movement, that's a good headline theme"). BUMPED deliberately, the is4 precedent: prompt_version keys the wrap's own input_hash, so the bump self-backfills exactly one wrap per live-wire entity as its pipeline next wakes — cheap (the wrap is the smallest call on the rail, live-wire entities only), and the alternative leaves every banked card hook-less until its board happens to move. The dead-wire no-row rule is untouched: no board, no call, the Veil. // is4, the PLAIN-TEXT pass (2026-08-22): the wire READ was shipping Markdown to the card. MEASURED on production `/transfers.wire_read`: "West Ham's wire is **one live, advanced call**" — four bold runs in one read, and the same shape across the sampled fleet. CAUSE, and it is a doc-comment as much as a prompt: this READ was audit-and-memory only, so it never picked up the plain-text rule every SERVED seat has (Analyst s8, Influencer v15, and the `**` entries in `RATING_BODY_BANS`/`VIBE_BODY_BANS`); drop 1 put it on the card as `wire_read` and did not carry the hygiene across, while the const's own doc still said "never served". Nothing surfaced it because the reply is JSON-schema constrained — `read` is a well-formed string dressed or not, so no parse ever broke. Two moves: (a) the rule lands at the EMISSION SITE, in the READ contract line and again inside the JSON shape (the momentum-s12/s18 lesson — a rule buried in a list loses to the line the model is actually writing); (b) `parse_insider_score_reply` now runs `util::strip_markdown_emphasis`, so the card is clean regardless of what the model does. Deliberately a STRIP, not a ban: a `**` rejection would fail nearly every wire read until the rule took, which is precisely the fail-rate mistake the same-day momentum-s18 / vibe-v21 pass exists to undo, and the helper's own rationale holds — asking a model not to emit Markdown is a request, not a guarantee, and every model swap re-runs the experiment. The version bump is load-bearing beyond the prompt: `prompt_version` is in the debounce pre-image, so it regenerates the rows already banked with decoration rather than leaving dressed prose on cards until each entity's board happens to change. Voice, length allowance, anti-fabrication rule, SCORE bands: untouched. // s9/or7/v16/n15/s16/is3 — the ALLOWANCE pass: the ceiling goes to eight sentences and is reframed as a platform allowance rather than a target. Measured cause: at a 5-6 floor the model reached for length, and the manufactured closing hedges then dragged the verdict (momentum scored -1 on a RISING entity off 'for now, this isn't a surge'). Brevity is now explicitly blessed — two sentences is a complete read. Plus an anti-fabrication rule: on a rumour wire, padding IS misinformation. is2: the peer-length pass — the wire READ grows from 1-2 to 5-6 sentences. Deliberately scoped to the Insider's own voice card: the per-rumor `summary` in TRANSFER_SYSTEM_PROMPT stays one tight sentence, because that field is structured data the Journalist and Oracle consume, not the character speaking.
...
///
/// The read is persisted for audit and memory AND SERVED — drop 1 put it on the Insider's card
...
FIRST, THE READ. Your claims are THE BOARD: the claim states what the whole wire holds, the evidence sentences are the credible calls — counterparty and stage, and why they are the credible ones — and the close lands whether the board is quickening or settling. Most wires are ONE claim, and one claim is a complete filing; only a board genuinely carrying two stories earns two. Only facts from the board shown; never invent a fee, stage, or source. The word "heat" and its numbers are internal; never recite them. PLAIN TEXT ONLY — this prints on a card exactly as you type it: no Markdown, no asterisks, no bold, no bullets, no headings. Emphasis is carried by which call you lead with, never by decoration.
...
THEN, THE HEADLINE. {headline} It is your read of the WHOLE wire, never a single rumor's summary restated: a quickening board names what is quickening; a quiet wire says the wire is quiet, plainly — that is a real headline, not a failure to find one.
...
- The headline and the score answer the same question in two registers, so they must agree.
...
{{"read": "<the READ — your claims in THE FORM, paragraphs separated by a blank line, plain text, no Markdown>", "headline": "<the hook — a tweet, 140 characters at most>", "score": <integer 1-99>}}"#,
            selection = crate::junctions::form::CLAIM_SELECTION,
            form = crate::junctions::form::STORY_FORM,
            wire = crate::junctions::form::WIRE_COPY,
            card = crate::junctions::form::card_face("headline", "the read"),
            headline = crate::junctions::form::headline_contract("The `headline`", "your sharpest claim about the wire")
        )
    },
);
...
/// `headline` THEN `score` (is5): the model reads the wire first, titles it second, and lands
/// the verdict last; range is 1-99 (the tarot deck's display scale), never the crown's 1-100.
