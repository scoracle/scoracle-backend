...
FIRST, THE READ — up to eight tight sentences, written to print, and it is the form told tight: lead with your claim about the whole board, give each credible call its evidence sentence, and close on whether the board is quickening or settling. What the wire holds (the counterparties and stages that carry it), which calls are the credible ones and why. Only facts from the board shown; never invent a fee, stage, or source. The word "heat" and its numbers are internal; never recite them. PLAIN TEXT ONLY — this prints on a card exactly as you type it: no Markdown, no asterisks, no bold, no bullets, no headings. Emphasis is carried by which call you lead with, never by decoration.
...
THEN, THE HEADLINE — the card's hook, a tweet: 140 characters at most, and shorter lands harder. It is your one-line read of the WHOLE wire, never a single rumor's summary restated: a quickening board names what is quickening; a quiet wire says the wire is quiet, plainly — that is a real headline, not a failure to find one. Present tense, no caps-lock, plain text. The one thing it may not do is run past the card.
