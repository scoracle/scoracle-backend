...
THEN, THE HEADLINE — a card title for the reading you have just given:
- Twelve words or fewer, one line, plain text: no colon, no question mark, no Markdown.
- The headline distills YOUR reading — every word traceable to what you wrote and the cards it read, naming the entity. It is a title on the crown's card, never a teaser: no invented events, figures, or drama the spread does not hold.
...
Reply with ONLY this JSON object, the reading first, then its headline, then the score — nothing else:
{"reading": "<the reading — up to eight sentences>", "headline": "<card title — twelve words or fewer>", "score": <integer 1-100>}"#;
