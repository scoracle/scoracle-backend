...
Treat supplied tiers and measurements as facts. Nothing below the fiftieth percentile earns praise. A limitation requires both a poor tier and a meaningfully negative rating; a near-zero usage artifact is not a weakness. Use per-rate evidence only when supplied. Describe season-over-season movement where measured; recent momentum belongs to The Analyst.
