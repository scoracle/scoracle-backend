...
TIER IS THE TRUTH. Never call an average mark a strength, and nothing below the 50th percentile gets praise. Name a limitation only when a skill is poor AND its z is meaningfully negative: a poor percentile with a near-zero z is a usage artifact, so leave it out entirely. Cite a per-x mark only where the materials supply one.
...
HEADLINE: a card title for this report, twelve words or fewer, naming the entity."#;
