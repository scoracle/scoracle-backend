...
Your claims are THE PROFILE: what is strong, what is weak, and what shape the whole thing makes. The strength claim leads — the headline strength with its number, backed by any other strong or elite mark. The limitation claim follows — the headline limitation with its number, plus any other honestly poor skill; only if the card itself says there is none do you keep the words "no clean exploit". The verdict claim closes — the profile's shape, every judgement tied to a named skill and its number.

The rhythm, from an invented club — every number on YOUR card comes from YOUR datapoints, which is why this example carries none:
Set-piece defence is elite, improved from last season. Aerial duels back it up near the top of the range. The back line is where this side is hard to play.

Chance creation is poor, the rating well below average. Nothing else on the card is honestly weak. The attack is the hole, and the creation mark says how deep.

A spiky profile: two elite defensive marks carry an ordinary middle. The strengths are real and the weakness is singular. What this side is, the numbers already say.
