...
/// The profile's SHAPE, read deterministically off the percentile distribution — the
/// tier/omen pattern (North Star #4: deterministic facts are computed, models narrate)
/// applied to the whole card. The shape WORD and the band rosters are code's; the Scout's
/// first claim voices them. Elite ≥ 90th pct, poor ≤ 25th, the middle is the 25th-75th band.
pub fn profile_shape_line(facts: &[super::RatingDatapoint]) -> String {
    let elite: Vec<&str> = facts
        .iter()
        .filter(|d| d.pct >= 90.0)
        .map(|d| d.label.as_str())
        .collect();
    let poor: Vec<&str> = facts
        .iter()
        .filter(|d| d.pct <= 25.0)
        .map(|d| d.label.as_str())
        .collect();
    ...
    let middle = facts
        .iter()
        .filter(|d| d.pct > 25.0 && d.pct < 75.0)
        .count();
    let word = match (!elite.is_empty(), !poor.is_empty()) {
        (true, true) => "split — real edges over real holes",
        (true, false) => "spiky — elite edges on an ordinary base",
        (false, true) => "leaky — no elite mark, and real holes",
        (false, false) => "flat — competent everywhere, exceptional nowhere",
    };
    let mut line = format!(
        "THE SHAPE (computed, final — like the tiers, you voice it, never re-derive it): {word}."
    );
    if !elite.is_empty() {
    ...
    line.push_str(&format!(
        " The middle: {middle} of {n} skills between the 25th and 75th percentile."
    ));
    line
}
...
Voice: thirty years of scouting. Clipped and specific. Every finding names a skill and cites its number; a generic line is a wasted line.
...
A finding on your card reads as the skill, its tier, the percentile, the rating — woven into the claim's prose, never as a "Label: value" line (the datapoint list is your evidence table, not your format). The rhythm, from an invented club, numberless so nothing can be copied, and deliberately plain — no aphorisms, no quotable closers (measured: a catchy example line ends up verbatim on real cards):
Harborview's card is split: one elite mark over two real holes, the rest ordinary. The elite mark is set-piece defence, near the top of the range and improved from last season. The holes are discipline and availability, both deep in the poor band, and together they undo half of what the back line wins.
The example deliberately shows no HEADLINE — yours is built from YOUR entity's name and YOUR report's sharpest claim, never from anyone else's.
...
THE SHAPE LINE IS COMPUTED AND FINAL — like the tier labels, you voice it, never re-derive it. Your FIRST claim voices the shape, told with its strongest evidence; your remaining claim or two are the biggest stories inside it. Summarise the ordinary middle in ONE sentence naming two or three marks; never walk it item by item.
...
Output, plain text with no Markdown of any kind: the report — your claims in THE FORM, paragraphs separated by blank lines, no labels of any kind — then the HEADLINE line last.
{headline}"#,
        ...
        headline = crate::junctions::form::headline_contract("HEADLINE", "the report's sharpest claim")
    )
});
...
pub fn build_stat_prompt(
    ...
    // THE SHAPE, computed (s28): the tier/omen pattern applied to the distribution itself.
    // Two live s27 cards proved a 3b cannot out-read a per-stat list — it mirrors it, one
    // paragraph per stat. So the shape is no longer the model's to derive: code reads the
    // distribution, the Scout voices it (deterministic facts are computed, models narrate).
    b.push('\n');
    b.push_str(&profile_shape_line(&ordered_facts(&p.breakdown)));
    b.push('\n');
