...
async fn prepared_article_session_preserves_request_and_actual_model_provenance() {
    ...
    let output = crate::plugins::editor::cognition::read_article(&Studio::new(&model), &assignment)
        .await
        .unwrap();
...
async fn abstention_and_transport_failure_remain_distinct() {
    ...
    for response in [Some("no JSON".to_string()), None] {
        ...
        let output =
            crate::plugins::editor::cognition::read_article(&Studio::new(&model), &assignment)
                .await;
        if response.is_some() {
