...
impl StudioPlugin for FixtureBoxscoreHandler {
    ...
    async fn execute(&self, item: &Item) -> Result<PluginOutcome> {
        ...
        let Some(fixture) = load_fixture(pool, fixture_id).await? else {
            return complete_unchanged(pool, item).await;
        };
...
async fn persist_record(
    ...
    insert_data_fetch_ledger(tx, fixture, &record).await?;
    publication.commit_final().await?;
    Ok(PluginOutcome::Committed)
}

async fn complete_unchanged(pool: &sqlx::PgPool, item: &Item) -> Result<PluginOutcome> {
