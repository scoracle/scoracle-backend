...
pub(super) async fn maybe_apply_transfer_identity(
    ...
    let options = crate::plugins::insider::cognition::identity_options(
        sport,
    ...
    if status == "applied" {
        let season: i32 =
            sqlx::query_scalar("SELECT current_season FROM public.sports WHERE id = $1")
                ...
                .fetch_one(&mut **tx)
                .await
                .context("load season for applied transfer rating obligation")?;
        let rating_input_version =
            crate::plugins::scout::adapter::rating_work_input_version_for_transfer(
                season,
                application_id,
            );
