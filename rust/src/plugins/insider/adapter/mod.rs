...
impl StudioPlugin for TransferHandler {
    ...
    async fn execute(&self, item: &Item) -> Result<PluginOutcome> {
        ...
        // Coach candidates are valid transfer subjects for pair vetting, but they are
        // `person` identities, not player identities. The wire-wrap product is keyed only
        // by player/team entities; treating every candidate as a player sends person ids
        // through the player identity loader and poisons the whole team claim with an
        // `essential memory identity` failure.
        for c in candidates.iter().filter(|c| c.subject_type == "player") {
