...
/// Subject selection for one non-queue metadata-adjudication invocation.
#[derive(Clone, Debug)]
...
/// Persistence policy for an operator-started factsweep. This is deliberately not a queue
/// claim: the sweep has no durable work item or claim-fenced publication boundary.
...
/// Run the Investigator's bounded person-metadata adjudication outside the durable queue.
/// The caller supplies already-resolved capabilities for the Investigator manifest.
pub async fn run_factsweep(
    pool: &PgPool,
    ...
    let sport = request.sport.to_uppercase();
    let dry_run = context == FactsweepRunContext::DryRun;
    let client = models.inference(crate::plugins::investigator::manifest::ROUTE)?;
    ...
    let candidates = load_candidates(pool, request, &sport).await?;
    println!(
    ...
    for cand in &candidates {
        stamp_checked(pool, cand.id, &sport, "checking", dry_run).await?;
        let ev = load_evidence(pool, cand.id, &sport).await?;
        if ev.len() < MIN_EVIDENCE_ARTICLES {
            thin += 1;
            stamp_checked(pool, cand.id, &sport, "thin evidence", dry_run).await?;
            continue;
        }
        ...
        let (result, _body) = match client.generate(&prompt, &opts).await {
            ...
            Err(error) => {
                ...
                stamp_checked(pool, cand.id, &sport, "model call failed", dry_run).await?;
                continue;
            }
        };
...
async fn load_candidates(
    ...
    let rows = if let Some(pid) = request.person {
        sqlx::query(
