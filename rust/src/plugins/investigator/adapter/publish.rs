    ...
    if pending {
        ...
        match decision {
            ...
            Decision::Accept {
                ...
                teams,
                plan,
            } => publish_accept(tx, candidate, sport, item, kind, *role, teams, plan).await?,
            Decision::Refuse {
                candidate,
                state,
                resolved,
                plan,
                reason,
            } => publish_refusal(tx, candidate, state, resolved.clone(), plan, reason).await?,
            Decision::Player {
                player_id,
                sport,
                it,
            } => publish_player(tx, *player_id, sport, it).await?,
            Decision::Team {
                team_id,
                sport,
                venue,
                logo,
            } => publish_team(tx, *team_id, sport, venue, logo).await?,
        }
    }
