...
pub fn parse_vibe_reply(raw: &str) -> Result<(i32, Option<String>, String)> {
    if raw.trim_start().starts_with('{') {
        let card: crate::studio::form::CardReply = serde_json::from_str(raw)?;
        let score = card
            .score
        ...
        return Ok((
            score,
            Some(card.headline),
            crate::studio::form::normalize_body(&card.body),
        ));
    }
    ...
    let body = crate::studio::form::normalize_body(body);
    if body.is_empty() {
...
/// Unlike Analyst, empty material publishes an uncalled marker, not NoMaterial.
pub async fn run<P: Publisher<VibeScore>>(
    studio: &Studio<'_>,
    assignment: &Assignment,
    publisher: &P,
) -> Result<P::Receipt> {
    let output = create(studio, assignment).await?;
...
pub fn generation_options(temperature: f64, num_ctx: i32, num_predict: i32) -> GenerateOptions {
    GenerateOptions {
        ...
        num_ctx,
        json_mode: false,
        format_schema: Some(crate::studio::form::card_schema(true)),
