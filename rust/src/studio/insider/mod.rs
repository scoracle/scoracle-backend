...
fn row_from_verdict(
    ...
    match verdict.and_then(|value| value.is_rumor) {
        ...
        Some(true) => {
            ...
            let summary = crate::studio::guards::clean_served_prose(&verdict.summary);
            (
                TransferRow {
                    is_rumor: Some(true),
                    direction: Some(direction_for(relationship).to_string()),
...
fn parse_insider_score_reply(raw: &str) -> Option<InsiderScoreReply> {
    ...
    let read =
        crate::studio::guards::clean_served_prose(&crate::studio::form::normalize_body(read));
    if read.is_empty() {
        return None;
    }
    Some(InsiderScore {
        read,
        headline: crate::studio::guards::settle_title(
            "insider",
...
impl Parser<InsiderScore> for InsiderScoreParser {
    fn parse(&self, raw: &str) -> Result<Option<InsiderScore>> {
        if let Ok(value) = serde_json::from_str::<serde_json::Value>(raw) {
            crate::studio::form::validate_hook(value.get("headline").and_then(|v| v.as_str()))?;
        }
        let reply = parse_insider_score_reply(raw)
            .ok_or_else(|| anyhow::anyhow!("insider score: could not parse read+score"))?;
        crate::studio::form::validate_body(&reply.read)?;
        Ok(Some(reply))
    }
}
