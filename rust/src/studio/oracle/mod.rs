...
mod brief;
mod inputs;
pub use crate::studio::form::oracle_format_schema;
...
pub fn parse_crown_reply(raw: &str) -> Option<CrownReply> {
    ...
    let reading = crate::studio::form::normalize_body(reading);
    // Served prose takes the shared scrub.
    let reading = crate::studio::guards::clean_served_prose(&reading);
    if reading.is_empty() {
        return None;
    }
...
pub use crate::studio::guards::count_sentences;
...
impl Parser<CrownReply> for CrownParser {
    fn parse(&self, raw: &str) -> Result<Option<CrownReply>> {
        match parse_crown_reply(raw) {
            Some(mut r) => {
                crate::studio::form::validate_body(&r.reading)?;
                crate::studio::form::validate_hook(r.headline.as_deref())?;
                // Global served-prose invariants fail closed and retry the item.
                if crate::studio::guards::has_bookkeeping_citation(&r.reading) {
                ...
                r.headline = crate::studio::guards::settle_title("oracle", r.headline.as_deref());
                if let Some(p) = crate::studio::guards::first_product_name(&r.reading) {
                    tracing::warn!(guard = "product_name", name = p, "reading rejected");
                    bail!("crown: reading names product {p:?}");
                }
                if crate::studio::guards::has_foreign_script(&r.reading) {
...
pub async fn create(studio: &Studio<'_>, assignment: &Assignment) -> Result<SigilOutput> {
    ...
    if !crate::studio::guards::title_names_entity(&reply.reading, &assignment.subject.entity_name) {
