...
# a durable `sigil` pipeline_work item for each. The always-on Rust cognition daemon
# drains the queue — resolving current_season and hash-gating the model call — so the
...
# Each run is recorded in pipeline_runs. Exit codes: 0 = success/overlap-skip;
# 3 = partial (some enqueues failed); 1 = all sports failed to enumerate.
#
