...
-- Player profile — stats only (no JOIN to public.players, public.teams, or
-- public.leagues). Name/headshot/etc. served by the meta endpoint; frontend
-- caches them locally for fast widget hydration.
...
COMMENT ON VIEW football.player IS
    'Football player profile — stats only. Meta (name, headshot, etc.) served separately and cached on the frontend.';
...
COMMENT ON FUNCTION football.stat_leaders IS
    'Returns top N Football players by stat category with positional filtering. Position sourced from player_stats.';
