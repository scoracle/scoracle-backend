-- 030_rating_breakdown.sql
--
-- Per-datapoint transparency for the rating engine. The z of each individual
-- datapoint that feeds Composite/Specialist is computed inside compute_rating
-- (the `z` CTE) and currently DISCARDED. This persists it as a JSONB column so
-- the profile's Composite tab (pizza of the composite datapoints) and Specialist
-- tab (peak skill + scarcity) can render it.
--
-- Core principle: the backend stores the composite (raw z, the engine's
-- currency) and serves it to the frontend as a PERCENTILE. Each datapoint object
-- carries `pct` = percent_rank of sign*z within the (sport, season, label)
-- population × 100 — the 0-100 the frontend actually draws. `sign*z` makes
-- negative datapoints (Ball Security, Giveaways, Possession Lost) read correctly:
-- low turnovers → high pct.
--
-- New column (player_stats, team_stats):
--   rating_breakdown JSONB — array, one object per datapoint:
--     {label, z, pct, in_comp, in_spec, sign, facet, is_specialty}
--
-- STRICTLY ADDITIVE: compute_rating / compute_team_rating are re-declared
-- verbatim with ONE new pass appended after the existing rank UPDATE (reusing
-- the still-live _rating_dp / _team_dp temp tables). The composite / specialist /
-- specialty / rank math is byte-identical — verified by a before/after board
-- diff. finalize_fixture is untouched: it already PERFORMs compute_rating /
-- compute_team_rating, so the breakdown rides the existing in-season recompute.

BEGIN;

ALTER TABLE player_stats ADD COLUMN IF NOT EXISTS rating_breakdown JSONB;
ALTER TABLE team_stats   ADD COLUMN IF NOT EXISTS rating_breakdown JSONB;

-- ---------------------------------------------------------------------------
-- compute_rating — 027's body verbatim + a final breakdown pass.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    scored AS (
        SELECT player_id, league_id, label, in_comp, in_spec, sign, facet, zr,
...
-- ---------------------------------------------------------------------------
-- compute_team_rating — 027's body verbatim + the breakdown pass (flat, no facet).
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    z AS (
        SELECT d.team_id, d.league_id, d.label, d.in_comp, d.in_spec, d.sign,
               COALESCE((d.value - p.mean) / p.sd, 0) AS zr
        FROM _team_dp d JOIN pop p USING (label)
    ),
    scored AS (
        SELECT team_id, league_id, label, in_comp, in_spec, sign, zr,
               ROUND((percent_rank() OVER (PARTITION BY label ORDER BY sign * zr ASC))::numeric * 100, 1) AS pct
        FROM z
    ),
    peak AS (
...
-- ---------------------------------------------------------------------------
-- Backfill: re-rate every (sport, season). compute_rating / compute_team_rating
-- now also write rating_breakdown, so this populates the new column. The
-- composite/specialist/rank values are recomputed identically (idempotent).
-- ---------------------------------------------------------------------------
DO $$
DECLARE r RECORD;
BEGIN
