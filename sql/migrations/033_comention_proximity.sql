-- 033_comention_proximity.sql
--
-- Co-mention PRECISION. Bare co-occurrence (team + player both linked to the same
-- article) over-links multi-subject roundups: a headline like
...
-- thirdparty.FirstMatchPos). A team↔player pair counts as a genuine co-mention
-- only when their mentions are within COMENTION_PROXIMITY_CHARS (50) of each other.
-- Symmetric (no "primary" anchor needed), so it also handles roundups where neither
-- is the queried entity. NULL title_pos (legacy / purged entity / not-in-title) is
-- treated leniently — the pair still counts — so the gate never silently drops data
-- it has no position for; run cmd/comention-backfill to populate history.
--
-- Window rationale: a real transfer pair is near-adjacent in the title ("Chelsea
-- target Delap", "Delap to Chelsea", "Everton want … Delap"); 50 chars (~7 words)
-- keeps those and drops cross-clause roundup artifacts. Tunable.

BEGIN;

ALTER TABLE news_article_entities
    ADD COLUMN IF NOT EXISTS title_pos SMALLINT;  -- char offset in the title; NULL = unknown

COMMENT ON COLUMN news_article_entities.title_pos IS
    'Character offset of this entity in the article title (thirdparty.FirstMatchPos); NULL = unknown. Powers the co-mention proximity gate (migration 033).';

-- ---------------------------------------------------------------------------
-- compute_transfer_heat — now proximity-gated on the news side. (Tweets carry no
-- position; they pass through unchanged — short text, not roundup-prone.)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION compute_transfer_heat(
    ...
    WITH corpus AS (
        -- News articles linking BOTH the team and the player, in PROXIMITY.
...
-- ---------------------------------------------------------------------------
-- seed_transfer_rumors — candidate discovery now proximity-gated too, so a player
-- only becomes a candidate when co-mentioned NEAR the team (not merely in the same
-- roundup). Mirrors the live loadCandidates gate in ml/transfer.go.
-- ---------------------------------------------------------------------------
