-- ============================================================================
-- 038_rating_breakdown_value.sql
-- Add raw `value` (the VOLUME) to each rating_breakdown datapoint, so the data
-- package carries the underlying counting stat next to z + pct. "Volume of each
-- datapoint we use in the z-score" — computed in the backend, served ready-made.
--
-- STRICTLY ADDITIVE: compute_rating (from 030) + compute_team_rating (from 037)
-- copied verbatim; the breakdown pass's z/scored CTEs now carry the datapoint
-- value and the JSONB gains 'value'. Composite/specialist/category math untouched.
-- ============================================================================
...
-- Backfill so existing rows gain the `value` field in their breakdown.
DO $$
DECLARE r RECORD;
BEGIN
