-- ============================================================================
-- 073 — Football: Shooting = shots ON TARGET only (drop raw/off-target shots).
--
...
-- ── 4. Gate: player Shooting == shots_on_target; team defense has SoT Allowed only.
DO $$
DECLARE v_sot INTEGER; v_old INTEGER;
BEGIN
    ...
    WHERE ps.sport='FOOTBALL' AND ps.season=2025 AND ps.position<>'Goalkeeper' AND el->>'label'='Shooting'
      AND (el->>'value')::numeric = (ps.stats->>'shots_on_target')::numeric AND (ps.stats->>'shots_on_target')::numeric > 0;
    SELECT count(*) INTO v_old
    FROM team_stats ts CROSS JOIN LATERAL jsonb_array_elements(ts.rating_breakdown) el
    WHERE ts.sport='FOOTBALL' AND ts.rating_breakdown IS NOT NULL AND el->>'label'='Shots Allowed';
    IF v_sot = 0 THEN RAISE EXCEPTION '073 FAIL: player Shooting not shots_on_target'; END IF;
    IF v_old > 0 THEN RAISE EXCEPTION '073 FAIL: % stale blended Shots Allowed remain', v_old; END IF;
    RAISE NOTICE '073 OK: Shooting = shots on target only (players + teams); defense SoT Allowed';
END $$;
