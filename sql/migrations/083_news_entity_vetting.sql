...
--
-- Scrub vetting columns on the fuzzy link table. The Gemma ID-gate (Stage 1,
...
--   vetted = FALSE  fuzzy false positive (Gemma dropped it)
...
-- vetted TRUE; ml/news_scrub.go never lets Gemma drop it.
--
...
COMMENT ON COLUMN news_article_entities.vetted IS
    'Gemma scrub verdict: TRUE genuinely the subject, FALSE fuzzy false positive, NULL not yet scrubbed. See ml/news_scrub.go.';
COMMENT ON COLUMN news_article_entities.scrubbed_at IS
    'When the Gemma scrub last judged this link (NULL = unscrubbed). Drives the async scrub worker backlog query.';
