...
--
-- Sigil convergence — the tail. Renames the local model-divined peak-strength label
...
-- column (it is local model-WRITTEN, not engine-computed — verified), so there is no
...
--                     `vibesynth --restamp-vocab` (no local model re-synthesis)
...
COMMENT ON COLUMN stat_summaries.divined_peak IS
    'local model-divined peak-strength label (e.g. "Rim Protection"), parsed from the rating prompt''s "PEAK: <label>" first line; the Rating card hero label. Was divined_sigil pre-convergence.';
