...
-- records its verdict beside Gemma's production `vetted` here, so we can measure agreement
...
--   gemma_vetted    — the production label this is validated against (news_article_entities.vetted)
--   cosine          — the article↔identity-card cosine the cheap CPU pre-filter banded on
--   band            — the pre-filter decision: keep / drop (auto, no model) | ambiguous (→ Gemma)
--   auto_verdict    — what the band ALONE decides (keep⇒true; drop/ambiguous-placeholder⇒false).
--                     For keep/drop links this EQUALS the hybrid verdict with ZERO Gemma — so the
--                     auto-decide safety metric (auto-keep precision, auto-drop FN rate) is a
--                     full-universe number that needs no GPU.
--   hybrid_verdict  — the FINAL resolve_set verdict. = auto_verdict for keep/drop links; the Gemma
...
--   decided_by      — provenance of hybrid_verdict: keep_band | drop_band | gemma | pending
...
--   model_version   — the adjudicator model id when decided_by='gemma' (else NULL).
--
...
CREATE TABLE IF NOT EXISTS resolve_shadow (
    ...
    decided_by         text        NOT NULL
                                   CHECK (decided_by IN ('keep_band', 'drop_band', 'gemma', 'pending')),
...
COMMENT ON TABLE resolve_shadow IS
    ...
    'resolve_set verdict beside Gemma''s vetted label, over the whole secondary-link corpus. '
