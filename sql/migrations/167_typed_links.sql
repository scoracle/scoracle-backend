-- 167_typed_links.sql
--
-- Junction memory rollout step 8: typed event → link aggregation. The graph stage
-- (mig 165) writes atomic narrative_events; this rolls them into typed narrative_links
-- rows (link_type = predicate — the mig-154 CHECK already admits all six), with
-- confidence weighting (confirmed > reported > speculative), the house ±10 trajectory,
-- and zero-decay out of window. Typed links are DIRECTIONAL (the canonical-ordering
-- constraint applies only to co_mention). roll_narrative_episodes is link_type-generic,
-- so typed links inherit episode memory for free the night they cross the threshold.
--
-- Second half: score_transfer_likelihood v2 — the language input (0.35 weight) upgrades
-- from legacy-only to GREATEST(legacy rumor-stage signal, typed event signal). The
-- probe-measured fact behind the whole graph build: language led volume by five weeks
-- on Rogers. Typed events are sparse today, so GREATEST degrades gracefully to legacy;
-- as typed volume matures the legacy input can retire by a later migration.
-- Components record both signals + which won, so the calibration series stays
-- explainable across the formula change (day-1 of accrual — the cheapest moment).
--
-- ADDITIVE — apply before the nightly chain runs refresh_typed_links (cron change
-- ships with this migration).

BEGIN;

CREATE OR REPLACE FUNCTION public.refresh_typed_links(
    p_sport text,
    p_window_days integer DEFAULT 90,
    OUT links_upserted integer,
    OUT links_zeroed integer
) RETURNS record
    LANGUAGE plpgsql
    AS $$
DECLARE
...
COMMENT ON FUNCTION public.refresh_typed_links(text, integer, OUT integer, OUT integer) IS
    'Rolls narrative_events into typed narrative_links (link_type = predicate) with '
    'confidence weighting, recency decay, the shared trajectory vocabulary, and '
    'zero-decay out of window. Directional (no canonical ordering). Run nightly after '
    'refresh_co_mention_links; roll_narrative_episodes is link_type-generic, so typed '
    'links earn episodes the night they cross the open threshold.';

CREATE OR REPLACE FUNCTION public.score_transfer_likelihood(p_sport text, p_max_rise_uncorroborated integer DEFAULT 10, p_max_fall integer DEFAULT 8, p_corrob_sources integer DEFAULT 3, p_corrob_tier numeric DEFAULT 0.7, OUT episodes_scored integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_run timestamptz := clock_timestamp();
...
COMMENT ON FUNCTION public.score_transfer_likelihood(p_sport text, p_max_rise_uncorroborated integer, p_max_fall integer, p_corrob_sources integer, p_corrob_tier numeric, OUT episodes_scored integer) IS
    'Fuses coverage + language (GREATEST of legacy rumor-stage and typed-event signal, '
    'mig 167) + source tier + trajectory (+ earned early-caller bonus) into the served '
    'transfer likelihood on each open player-team episode, with hysteresis. Components '
    'record both language signals and which won. Run after roll_narrative_episodes.';

INSERT INTO public.schema_migrations(version) VALUES ('167_typed_links')
