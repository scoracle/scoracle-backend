-- 246: the aggregators emit whole numbers whole — the expression-index cast.
--
-- 245's ROUND(total, 2) serialized counts as 3.00, and idx_team_stats_points
-- evaluates ((stats->>'points')::integer) at insert — '3.00' does not cast.
-- Every whole-valued total now serializes as an integer; only genuinely
-- fractional totals (xG, ICT) keep two decimals. Same shape as the vendor era
-- wrote, so downstream int casts keep working for both eras.
--
