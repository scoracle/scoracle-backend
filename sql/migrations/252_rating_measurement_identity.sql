-- Preserve a skill's underlying measurement across provider eras.
-- Derived from the live functions on 2026-09-13. Measured numeric values are unchanged;
-- absent Shooting/Chance Creation sources stay NULL, not an invented zero.
-- Rate modes require their denominator and rate inputs: no raw-total fallback.
-- Existing rating_datapoints contracts remain six-column adapters.
BEGIN;
...
DO $check$
BEGIN
    IF (SELECT measure FROM public.rating_measurements('FOOTBALL', '{"shots_on_target":32,"expected_goals":0}', 'total', 'Midfielder') WHERE label='Shooting')
        IS DISTINCT FROM 'shots on target' THEN RAISE EXCEPTION 'Shooting source priority changed'; END IF;
    IF (SELECT (value,measure) IS NOT DISTINCT FROM (0.14::numeric,'expected goals'::text)
        FROM public.rating_measurements('FOOTBALL', '{"expected_goals":0.14}', 'total', 'Midfielder') WHERE label='Shooting') IS NOT TRUE
        THEN RAISE EXCEPTION 'Expected goals lost measurement identity'; END IF;
    IF (SELECT measure FROM public.rating_measurements('FOOTBALL', '{"expected_assists":1.01}', 'total', 'Midfielder') WHERE label='Chance Creation')
        IS DISTINCT FROM 'expected assists' THEN RAISE EXCEPTION 'Expected assists lost measurement identity'; END IF;
    IF (SELECT measure FROM public.rating_measurements_team('FOOTBALL', '{"expected_goals_for":2,"assists":1}') WHERE label='Creation')
        IS DISTINCT FROM 'assists' THEN RAISE EXCEPTION 'Team creation lost measurement identity'; END IF;
END $check$;

INSERT INTO public.schema_migrations(version) VALUES ('252_rating_measurement_identity') ON CONFLICT DO NOTHING;
COMMIT;
